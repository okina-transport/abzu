...
import {createParkingHref} from "../../utils";

class NewParkingInfo extends React.Component {
    handleClose() {
        this.props.dispatch(UserActions.closeSuccessfullyCreatedNewStop());
    }

    render() {
        const { open, parkingId, intl } = this.props;
        const { formatMessage } = intl;
        const translations = {
            close: formatMessage({ id: 'close' }),
            newParkingCreated: formatMessage({ id: 'new_stop_created' }),
            newParkingCreatedMore: formatMessage({ id: 'new_stop_created_more' }),
        ...
        const href = createParkingHref(parkingId);
        ...
        return (
            <div style={informationBannerStyle}>
                <div style={titleStyle}>
                    ...
                    />
                    {translations.newParkingCreated}
                </div>
                <div style={bodyStyle}>{'ID: ' + parkingId}</div>
                <div style={ingressStyle}>{translations.newParkingCreatedMore}</div>
...
export default injectIntl(connect(null)(NewParkingInfo));
