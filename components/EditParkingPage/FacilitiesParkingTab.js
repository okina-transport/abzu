...
import ToolTipIcon from '../EditStopPage/ToolTipIcon';
...
class FacilitiesParkingTab extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      expandedIndex: -1,
  ...
  handleTicketMachineChange(value) {
    ...
    if (!disabled) {
      dispatch(EquipmentActions.updateTicketMachineStateParking(value, 'parking', index));
    }
  }
  ...
  handleBusShelterChange(value) {
    ...
    if (!disabled) {
      dispatch(
        EquipmentActions.updateShelterEquipmentStateParking(value, 'parking', index),
      );
    }
  }
  ...
  handleValueForBusShelterChange(newValue) {
    ...
    const oldValuesSet = {
      seats: getIn(parking, ['placeEquipments', 'shelterEquipment', 'seats'], 0),
      stepFree: getIn(
        parking,
  ...
  handleWCChange(value) {
    const { index, disabled, dispatch } = this.props;
    if (!disabled) {
      dispatch(EquipmentActions.updateSanitaryStateParking(value, 'parking', index));
    }
  }
  ...
  handleWaitingRoomChange(value) {
    const { index, disabled, dispatch } = this.props;
    if (!disabled) {
      dispatch(EquipmentActions.updateWaitingRoomStateParking(value, 'parking', index));
    }
  }
  ...
  handle512Sign(value) {
    if (!this.props.disabled) {
      this.props.dispatch(
        EquipmentActions.update512SignStateParking(value, 'parking', this.props.index),
      );
    }
  }

  render() {
    const { intl, disabled, parking } = this.props;
    const { formatMessage } = intl;
    const { expandedIndex } = this.state;

    const ticketMachine = equipmentHelpers.getTicketMachineState(parking);
    const busShelter = equipmentHelpers.getShelterEquipmentState(parking);
    const waitingRoom = equipmentHelpers.getWaitingRoomState(parking);
    const WC = equipmentHelpers.getSanitaryEquipmentState(parking);
    ...
    const sign512 = equipmentHelpers.get512SignEquipment(parking);
...
export default connect(null)(FacilitiesParkingTab);
