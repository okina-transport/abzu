...
docker build --tag=registry.okina.fr/na/abzu:${PACKAGE_VERSION} --no-cache  --force-rm=true . --build-arg SSH_KEY="$(cat ~/.ssh/id_rsa)"
docker push registry.okina.fr/na/abzu:${PACKAGE_VERSION}